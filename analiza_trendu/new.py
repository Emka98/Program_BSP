...
# Import jest już u Ciebie na górze, upewnij się tylko, że plik jest zaktualizowany
...
def make_prediction(df: pd.DataFrame, 
    ...
    text += f"Obciązenie powierzchni: {pred_wl:0.3f}\n"
    ...
    text += f"Obciązenie mocy: {pred_pl:0.3f}\n"
    ...
    # =========================================================================
    # TUTAJ WYWOŁUJEMY FUNKCJĘ ODLEGŁOŚCI EUKLIDESOWEJ
    # =========================================================================
    path_similarity_result = os.path.join(path_result, "najbardziej_zblizone_modele.txt")
    
    odleglosc_euklidesowa(
        df=df,
        t_endurance=pred_endurance,
        t_range=pred_range,
        t_ar=pred_ar,
        t_wl=pred_wl,
        t_pl=pred_pl,
        year_of_prediction=prediction_year,
        top_n=5,  # Liczba modeli, którą chcesz wyciągnąć (możesz zmienić)
        output_filename=path_similarity_result
    )
    # =========================================================================
